  ...
  <div class="general_box">
    ...
    <div class="general_table">
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="meeting" label="会议/批示时间" />
        <el-table-column prop="involve" label="涉及人员" />
      ...
      >
        <div>
          ...
          >
            ...
            <el-form-item label="会议/批示时间" style="width: 48%">
              <el-input v-model="peoperForm.meeting" :disabled="dialogVisibleTitle == '查看概况'" />
            </el-form-item>
            <el-form-item label="涉及人员" style="width: 48%">
              <el-input v-model="peoperForm.involve" :disabled="dialogVisibleTitle == '查看概况'" />
            </el-form-item>
...
export default {
  ...
  methods: {
    ...
    fileSeeDialog(row) {
      ...
      row.instructionsBodyEntities.filter((item) => {
        item.name = item.filename
      })
      this.fileList = row.instructionsBodyEntities
    },
    // 文件上传
    carImgChange(e) {
      console.log(e)
      ...
      if (this.fileType == 0) {
        ...
        this.apiPostJson('instructions/savebody', form).then(
          (res) => {
            if (res.code == 200) {
              this.$message({
                message: '文件上传成功',
    ...
    handleRemove(item) {
      const that = this
      this.apiPost('instructions/delete', {
        id: item.id
      }).then((res) => {
        if (res.code == 200) {
          this.$message({
    ...
    inspectionSurveySelect() {
      this.apiPost('instructions/select', {
