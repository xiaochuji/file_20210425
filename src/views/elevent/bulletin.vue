      ...
      >
        <div>
          ...
          >
            ...
            <el-form-item label="网址" style="width: 48%">
              <el-input v-model="peoperForm.website" :disabled="dialogVisibleTitle == '查看概况'" />
            </el-form-item>
...
export default {
  ...
  methods: {
    ...
    fileSeeDialog(row) {
      ...
      row.journalismBodyEntities.filter((item) => {
        item.name = item.filename
      })
      this.fileList = row.journalismBodyEntities
    },
    // 文件上传
    carImgChange(e) {
      console.log(e)
      ...
      if (this.fileType == 0) {
        ...
        this.apiPostJson('journalism/savebody', form).then(
          (res) => {
            if (res.code == 200) {
              this.$message({
                message: '文件上传成功',
    ...
    handleRemove(item) {
      const that = this
      this.apiPost('journalism/delete', {
        id: item.id
      }).then((res) => {
        if (res.code == 200) {
          this.$message({
    ...
    inspectionSurveySelect() {
      this.apiPost('journalism/select', {
