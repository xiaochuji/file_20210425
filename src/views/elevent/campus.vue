      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="type" label="类型">
          <template slot-scope="scope">
            <span>{{ scope.row.type == 1 ? '通知' : scope.row.type == 2 ? '任命' : scope.row.type == 3 ? '反馈' : '成立临时党支部申请批复' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="filename" label="文件名称" />
        <el-table-column prop="fileNumber" label="文件号" />
      ...
      >
        <div>
          ...
          >
            ...
            <el-form-item label="文件名称" style="width: 48%">
              <el-input v-model="peoperForm.filename" :disabled="dialogVisibleTitle == '查看概况'" />
            </el-form-item>
            <el-form-item label="文件号" style="width: 48%">
              <el-input v-model="peoperForm.fileNumber" :disabled="dialogVisibleTitle == '查看概况'" />
            </el-form-item>
            <el-form-item label="时间" style="width: 48%">
              <el-date-picker
                v-model="peoperForm.fileTime"
                type="date"
                placeholder="选择日期"
...
export default {
  ...
  methods: {
    ...
    fileSeeDialog(row) {
      ...
      row.documentBodyEntities.filter((item) => {
        item.name = item.filename
      })
      this.fileList = row.documentBodyEntities
    },
    // 文件上传
    carImgChange(e) {
      console.log(e)
      ...
      if (this.fileType == 0) {
        ...
        this.apiPostJson('document/savebody', form).then(
          (res) => {
            if (res.code == 200) {
              this.$message({
                message: '文件上传成功',
