  ...
  <div class="general_box">
    ...
    <div class="general_table">
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="closure" label="距现场巡视结束天数" />
...
export default {
  ...
  methods: {
    ...
    fileSeeDialog(row) {
      ...
      row.patrolLeaderBodyEntities.filter((item) => {
        item.name = item.filename
      })
      this.fileList = row.patrolLeaderBodyEntities
    },
    // 文件上传
    carImgChange(e) {
      console.log(e)
      ...
      if (this.fileType == 0) {
        ...
        this.apiPostJson('patrolLeader/savebody', form).then(
          (res) => {
            if (res.code == 200) {
              this.$message({
                message: '文件上传成功',
    ...
    handleRemove(item) {
      const that = this
      this.apiPost('patrolLeader/delete', {
        id: item.id
      }).then((res) => {
        if (res.code == 200) {
          this.$message({
    ...
    inspectionSurveySelect() {
      this.apiPost('patrolLeader/select', {
    ...
    onSubmit() {
      let url = ''
      if (this.isUpdate) {
        url = 'patrolLeader/update'
      } else {
        url = 'patrolLeader/save'
      }
