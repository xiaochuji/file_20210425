  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="所在单位" style="width: 48%">
        <el-input
          v-model="peoperForm.company"
          :disabled="dialogVisibleTitle == '查看员工'"
        />
      </el-form-item>
      <el-form-item label="参加巡查轮次" style="width: 48%">
        <el-input
          v-model="peoperForm.turn"
          :disabled="dialogVisibleTitle == '查看员工'"
          @input="peoperForm.turn = peoperForm.turn.replace(/[^\d]/g, '')"
        />
      </el-form-item>
      ...
      <el-form-item
        v-if="
          dialogVisibleTitle == '添加员工' || dialogVisibleTitle == '编辑员工'
        "
        style="width: 100%"
      >
...
export default {
  ...
  methods: {
    ...
    fileSeeDialog(row) {
      ...
      row.inspectionTeamLeaderBodyEntities.filter((item) => {
        item.name = item.filename
      })
      this.fileList = row.inspectionTeamLeaderBodyEntities
    },
    // 初始化获取
    inspectionSurveySelect() {
      this.apiPost('/inspectionTeamLeaderEntity/select', {
    ...
    carImgChange(e) {
      ...
      if (this.fileType == 0) {
        ...
        this.apiPostJson('inspectionTeamLeaderEntity/savebody', form).then(
          (res) => {
            if (res.code == 200) {
              this.$message({
                message: '文件上传成功',
    ...
    handleRemove(item) {
      const that = this
      this.apiPost('inspectionTeamLeaderEntity/delete', {
        id: item.id
      }).then((res) => {
        if (res.code == 200) {
          this.$message({
    ...
    onSubmit() {
      let url = ''
      if (this.isEdit) {
        delete this.peoperForm.inspectionTeamLeaderBodyEntities
        delete this.peoperForm.inspectionTeamLeaderBodyEntitySet
        url = '/inspectionTeamLeaderEntity/update'
      } else {
        url = '/inspectionTeamLeaderEntity/save'
      }
